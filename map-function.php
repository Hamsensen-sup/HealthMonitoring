...
function fetchTopDiseases($selectedPurok, $selectedYear) {
    global $conn;
    ...
    $query = "SELECT disease, COUNT(disease) as cases, MAX(sex) as mostAffectedGender
              FROM tblrecord
              WHERE street = ? AND year = ?
              GROUP BY disease
              ORDER BY cases DESC LIMIT 5";

    $stmt = $conn->prepare($query);
    $stmt->bind_param("si", $selectedPurok, $selectedYear);
    $stmt->execute();
    $result = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['yearlist'])) {
    $selectedYear = $_POST['yearlist'];
}

$topDiseases = fetchTopDiseases($selectedPurok, $selectedYear);

function fetchTotalDiseaseCases($selectedPurok, $selectedYear) {
    global $conn;
    $query = "SELECT COUNT(disease) as totalCases FROM tblrecord WHERE street = ? AND year = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("si", $selectedPurok, $selectedYear);
    $stmt->execute();
    $result = $stmt->get_result();
